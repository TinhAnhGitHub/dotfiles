---
name: databricks-official-aibi-dashboards
...
> **When a custom app fits better:** A managed AI/BI dashboard is the right tool for read-only KPIs, charts, and filters over governed tables. If the user instead needs a *custom-code interactive app* — write-back / data entry, bespoke UI or interactions beyond the dashboard grid, embedded or auth-gated workflows, or a conversational Genie/chat assistant as the primary surface — build a Databricks App instead with the `databricks-official-apps` skill (which brings in `databricks-official-app-design` for the data-screen UX). Linking an "Ask Genie" space to *this* dashboard stays here (see Linking a Genie Space below).
...
2. **Metric-view source** — if the dataset's `asset_name` (or `FROM` clause) is a UC metric view, its YAML-defined measures are already queryable. **Do not redeclare them** in `columns`. See [databricks-metric-views](../databricks-official-metric-views/SKILL.md).
...
- **[databricks-apps](../../skills/databricks-official-apps/SKILL.md)** - when the user needs a custom-code interactive app (write-back, bespoke UI, in-app chat / Genie) instead of a managed dashboard
- **[databricks-unity-catalog](../databricks-official-unity-catalog/SKILL.md)** - for querying the underlying data and system tables
- **[databricks-pipelines](../databricks-official-pipelines/SKILL.md)** - for building the data pipelines that feed dashboards
- **[databricks-jobs](../databricks-official-jobs/SKILL.md)** - for scheduling dashboard data refreshes
