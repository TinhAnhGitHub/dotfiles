...
## Ingesting into Qdrant Cloud

Use when: the ingestion or indexing happens against a managed cloud cluster (`*.cloud.qdrant.io`).

Cloud adds managed constraints and levers that change the ingestion playbook: strict mode (unindexed filtering rejected), `optimizer_cpu_budget`/GPU nodes configured in the Console instead of config files, managed resharding, and gRPC-only migration tooling.

See the [qdrant-cloud-ingestion](https://skills.qdrant.tech/md/cloud-ingestion/) skill for the full cloud indexing & ingestion guidance.
