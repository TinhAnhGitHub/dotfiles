...
| [references/model-serving-limits.md](references/model-serving-limits.md) | Resource/payload limits, rate limits per model (ITPM/OTPM/QPH), provisioned throughput limits, networking/security limitations, compliance standards, and region availability. |
| [references/scaling-provisioning.md](references/scaling-provisioning.md) | Auto-scaling behavior, provisioned concurrency calculation, scale-to-zero intricacies, GPU/CPU workload types, route optimization, express deployments, and queuing. |
| [references/production-optimization.md](references/production-optimization.md) | Production tuning: route optimization, instance selection, model optimizations (quantization/batching), client-side optimizations (connection pooling/retry), monitoring via system tables, and cost troubleshooting. |
| [references/foundation-model-apis.md](references/foundation-model-apis.md) | FM API specifics: per-model rate limits, pricing tables (DBU/M tokens), provisioned throughput pricing, output token limits, and best practices. |
| [references/cost-budget.md](references/cost-budget.md) | Cost controls: querying `system.billing.usage`, SKU names, budget policies (API/Terraform), cost attribution dashboards, and optimization strategies. |
| [references/ci-cd-deploy.md](references/ci-cd-deploy.md) | CI/CD patterns for model deployment: DABs-based deploy, GitHub Actions pipeline, automated canary deployment with traffic splitting, and Terraform IaC. |
| [references/agent-framework.md](references/agent-framework.md) | Databricks Agent Framework overview: AI Playground, Knowledge Assistant, custom agents, MCP support, agent evaluation, and production monitoring. Complements the `genai-agents.md` deep-dive. |
| [references/endpoint-config-sdk.md](references/endpoint-config-sdk.md) | Agent model serving endpoint infrastructure configuration via Python SDK: full CRUD, AI Gateway config, permissions, registered models in UC, provisioned throughput endpoints, notification settings, tags, logs/metrics, and agent endpoint lifecycle checklist. |
...
| Topic | REST API URL | Notes |
|-------|-------------|-------|
| Serving endpoints | `https://<host>/api/2.0/serving-endpoints` | Full CRUD, query, AI Gateway, permissions |
| Registered models (UC) | `https://<host>/api/2.1/unity-catalog/registered-models` | Unity Catalog model registry |
| Model versions (UC) | `https://<host>/api/2.1/unity-catalog/model-versions` | Version management |
| Model aliases | `https://<host>/api/2.1/unity-catalog/registered-models/{name}/aliases` | Alias CRUD |
| Experiments (MLflow) | `https://<host>/api/2.0/mlflow/experiments` | MLflow tracking |
| Runs (MLflow) | `https://<host>/api/2.0/mlflow/runs` | Experiment runs |
| External models | `https://<host>/api/2.0/endpoint-external-models` | External model providers |
| Permissions | `https://<host>/api/2.0/permissions` | Generic permissions API |

For the complete SDK method signatures and dataclass definitions, see [references/endpoint-config-sdk.md](references/endpoint-config-sdk.md).
